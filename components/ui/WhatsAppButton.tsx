...
export default function WhatsAppButton() {
  ...
  // Show label after 3 seconds to grab attention
  useEffect(() => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3">
      ...
      <AnimatePresence>
        {showLabel && (
          <motion.div
            ...
            className="bg-white rounded-2xl shadow-xl px-4 py-3 border border-green-100 max-w-[200px]"
          >
            <p className="font-poppins text-xs font-semibold text-charcoal leading-tight">
              💬 Chat with us on WhatsApp!
            </p>
            <p className="font-poppins text-[10px] text-warm-gray mt-0.5">
              Book in 60 seconds ⚡
            </p>
            {/* Triangle pointer */}
      ...
      {/* Main button */}
      <motion.a
        href="https://wa.me/923155072704?text=Hi%20GIA's%20Beauty%20Salon!%20I'd%20like%20to%20book%20an%20appointment."
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Chat on WhatsApp"
        onClick={() => setHasInteracted(true)}
        onMouseEnter={() => setShowLabel(true)}
        onMouseLeave={() => setTimeout(() => setShowLabel(false), 1500)}
